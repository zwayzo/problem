                    ...
                    if (FD_ISSET(i, &read_fds)){
                        ...
                        if (mycl[in - 1].post == 1){
                            ...
                            if (index == 0){
                                ...
                                mycl[in - 1].track += nbytes - z;
                            }
                            else{
                                mycl[in - 1].fileD.write(buf, nbytes);
                                mycl[in - 1].track += nbytes;
                            }
                            index++;
                        }   
                        if (nbytes > 0){
                        std::cout << "1-------------------------------------\n" << buf <<"\n2----------------------------------\n";
                        std::cout << "track is " << mycl[in - 1].track << " lenght is " << mycl[in-1].contentLenght << '\n';
                        usleep(100000);
                        if ( mycl[in - 1].track >= mycl[in - 1].contentLenght){
                            printf("time to clear\n");
                            clearSets(&mycl[in - 1], i, &mycl[in - 1].track, &index, &master_re, &master_wr);
                            }
                        }
